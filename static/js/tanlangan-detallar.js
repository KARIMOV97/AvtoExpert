...
document.querySelector('button[name="detallar-tasdiqlash"]').addEventListener("click", function () {
    ...
    fetch(`avtoexpert-production.up.railway.app/api/ichki-detail-coefficient/?car_model_id=${carModelId}`)
        .then(res => res.json())
        .then(data => {
            data.forEach(item => {
    ...
    fetch(`avtoexpert-production.up.railway.app/api/kuzov-detail-coefficient/?car_model_id=${carModelId}`)
        .then(res => res.json())
        .then(data => {
            data.forEach(item => {
